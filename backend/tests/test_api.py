...
class TestTheListingIsStrictlyChronological:
    """Nothing is promoted out of time order — the listing is grouped by day on the client.

    A video event used to lead the feed, which suited a card stack. Under day headers the
    same promotion puts a row beneath the wrong date, so these tests exist to stop it
    coming back: they are the ones that would have failed while that behaviour shipped.
    """
    ...
    def test_a_video_event_holds_its_place_in_time(self, client, db):
        ...
        assert self.names(client) == ["Photo only", "Has video"]

    def test_a_video_event_on_a_later_day_does_not_jump_days(self, client, db):
        """The case day grouping cares about: a promoted row would render under a header
        for a date it does not belong to."""
        early = datetime.combine(FAR_FUTURE, time(18), tzinfo=VEGAS_TZ)
        add_event(db, name="Tonight A", start=early)
        add_event(db, name="Tonight B", start=early + timedelta(hours=1))
        add_event(db, name="Tomorrow with video", start=early + timedelta(days=1),
                  video_url="https://media.vegasthisweekend.com/video/a.mp4")
        assert self.names(client) == ["Tonight A", "Tonight B", "Tomorrow with video"]

    def test_order_holds_when_nothing_has_video(self, client, db):
        early = datetime.combine(FAR_FUTURE, time(18), tzinfo=VEGAS_TZ)
    ...
    def test_the_order_does_not_change_from_one_request_to_the_next(self, client, db):
        """Paging is by offset, so a reshuffling order would duplicate and drop rows."""
        early = datetime.combine(FAR_FUTURE, time(18), tzinfo=VEGAS_TZ)
        ...
        assert self.names(client) == self.names(client)

    def test_paging_repeats_nothing_and_drops_nothing(self, client, db):
        early = datetime.combine(FAR_FUTURE, time(18), tzinfo=VEGAS_TZ)
        for index in range(4):
            add_event(db, name=f"Event {index}", start=early + timedelta(hours=index))
        add_event(db, name="Has video", start=early + timedelta(hours=9),
                  video_url="https://media.vegasthisweekend.com/video/a.mp4")
        ...
        assert seen == ["Event 0", "Event 1", "Event 2", "Event 3", "Has video"]
